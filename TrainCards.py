...
def StartTrain():
    ...
    for index, row in train_data.iterrows():
        fileTrain = "C:\\Users\dmitr\\Desktop\\DataSet\\Cards\\Valid\\train\\train\\" + row['filename']
        img = load_img(fileTrain, target_size=(970, 700)) # Укажите ширину и высоту
        img_array = img_to_array(img)
        train_rec = row['xmin'], row['ymin'], row['xmax'], row['ymax']
    ...
    for index, row in test_data.iterrows():
        ...
        test_labels.append(np.array(test_label))
    ...
    # Создание модели
    with tf.device('/GPU:0'): #  Указываем использование GPU
        model = Sequential()
